...
let homePage = [
/*	
// TO-DO: Figure out why icon doesn't display correctly
{
		tagName: "nav", className: "side-nav",
		children: [
			{tagName: "div", className: "icon"},
...
function parseElementDefinition(e) {
	...
	for ([property, value] of Object.entries(element.style)) {
...
function parsePageDefinition(page) {
...
window.onload = () => {
	...
	if (container) {
		container.appendChild(parsePageDefinition(homePage))
	}
}