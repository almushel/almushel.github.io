...
![Auth0 with Unstoppable Domains](../../images/login-selection-auth0.png)
...
In addition to the basic configuraton outlined in [Login Client Configuration](../login-client-configuration), some configuration specific to Auth0 is required.

1. Set your [Redirect URI](../login-client-configuration#redirect-uris) to `https://{{ TENANT_ID}} .{{ REGION }}.auth0.com/login/callback`. You can copy the origin from the **Domain** field of your Auth0 application **Settings**.
2. Set the [Token Endpoint Authentication Method](../login-client-configuration#token-endpoint-authentication-method) in the **Advanced** configuration page to **Client Secret Post**.
...
<div class="admonition warning">
The **Optional** and **Required** variations of each permission are mutually exclusive. Enabling both (e.g. `User profile (required)` and `User profile (optional)`), will result in an error. See [Optional Scopes](../../scopes-for-login#optional-scopes) for more information.
</div>

## Step 4: Add Custom Claims (Optional)

Some of the Web3 claims offered by Unstoppable Domains are outside the standard OAuth2 specification. For example, the `wallet_address` claim may be useful to your app but is not included in the default profile. To learn more about the available scopes for Login with Unstoppable Domains, see [Scopes for Login](../../scopes-for-login).
