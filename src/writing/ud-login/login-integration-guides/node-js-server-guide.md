...
# Unstoppable Login: Node.js Server 
...
Add `http://localhost:5000/callback` to your client's [Redirect URIs](../login-client-configuration#redirect-uris). Make sure that the port matches the port your app is being served to. Example code in this guide assumes port 5000.

Set the [Token Endpoint Authentication Method](../login-client-configuration/#token-endpoint-authentication-method) on your client dashboard to **Client Secret Post**. Once you've clicked the **Confirm Changes** button, `clientSecret` and `clientAuthMethod` will be added to your client metadata on the **Basic** configuration page.
...
```shell
npm intall --save express-session express express-async-errors morgan
```
...
The form must call the endpoint where the [`login` handler](#the-login-method) is called and it must correspond with the parameters to that function.
